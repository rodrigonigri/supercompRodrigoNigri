...
bool verificarSobreposicao(const vector<filme>& cronograma, const filme& novo_filme) {
    for (const filme& f : cronograma) {
        if ((novo_filme.inicio < f.fim) && (f.inicio < novo_filme.fim)) {
            return true; // Sobreposição de horários
        }
    }
